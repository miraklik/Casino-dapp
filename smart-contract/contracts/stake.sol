...
contract Stake is Ownable {
    ...
    uint256 public rewartPerTokenStored;
    ...
    modifier updateReward(address _account) {
        rewartPerTokenStored = rewardPerToken();
        updatedAt = lastTimeRewardApplicade();

        if (_account != address(0)) {
            rewards[_account] = earned(_account);
            userRewardsPerTokenPaid[_account] = rewartPerTokenStored;
        }
        _;
    }

    function lastTimeRewardApplicade() public view returns (uint256) {
        return _min(finishAt, block.timestamp);
    }

    function rewardPerToken() public view returns (uint256) {
        if (totalSupply == 0) {
            return rewartPerTokenStored;
        }

        return rewartPerTokenStored
            + (rewardRate * (lastTimeRewardApplicade() - updatedAt) * 1e18)
                / totalSupply;
    }
